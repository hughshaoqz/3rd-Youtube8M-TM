...
def main(unused_argv):
  ...
  total_written = 0
  total_error = 0
  for video_file, labels, segment_info in csv.reader(open(FLAGS.input_videos_csv)):
    rgb_features = []
    ...
    if FLAGS.insert_segment_info:
        ...
        """
        segment_start_times = segment_info['segment_start_times']
        segment_end_times = segment_info['segment_end_times']
        segment_labels = segment_info['segment_labels']
        segment_scores = segment_info['segment_scores']

        context_features['segment_start_times'] = tf.train.Feature(
            float_list=tf.train.FloatList(value=segment_start_times))
        context_features['segment_end_times'] = tf.train.Feature(
            float_list=tf.train.FloatList(value=segment_end_times))
        context_features['segment_labels'] = tf.train.Feature(
            float_list=tf.train.Int64List(value=segment_labels))
        context_features['segment_scores'] = tf.train.Feature(
            float_list=tf.train.FloatList(value=segment_scores))
