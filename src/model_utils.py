...
def ReshapeFramesToMIL(model_input):
  """Samples a random set of frames of size num_samples.
  ...
  batch_size = tf.shape(model_input)[0]
  paddings = [[0, 0], [0, FLAGS.max_frames-tf.shape(model_input)[1]], [0, 0]]
  model_input = tf.pad(model_input, paddings, 'CONSTANT', constant_values=0)
  #model_input = tf.reshape(model_input, [batch_size, FLAGS.max_frames//5, 5, 1152])
  model_input = tf.reshape(model_input, [batch_size, FLAGS.max_frames, 1152])
  return model_input
