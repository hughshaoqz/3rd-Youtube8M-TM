...
if __name__ == "__main__":
  ...
  flags.DEFINE_bool(
      "output_prob", False,
      "Output probability.")
  flags.DEFINE_integer("shift", 0, 'How many shift used to inference')
...
def inference(reader, train_dir, data_pattern, out_file_location, batch_size,
  ...
  with tf.Session(config=tf.ConfigProto(
    ...
    #"""
    #saver = tf.train.import_meta_graph(
    #    meta_graph_location, clear_devices=False)
    ...
    if FLAGS.segment_labels:
      ...
      if FLAGS.google_cloud:
        ...
        out_file = tempfile.NamedTemporaryFile(mode='w+', dir='G:/tmp')
      logging.info(
      ...
      if FLAGS.segment_label_ids_file:
        whitelisted_cls_mask = np.zeros((predictions_tensor.get_shape()[-1],),
                                        dtype=np.float32)
        with gfile.Open(FLAGS.segment_label_ids_file) as fobj:
          for line in fobj:
            try:
              cls_id = int(line)
    ...
    if FLAGS.output_prob:
      final_out_file.write("VideoId,LabelConfidencePairs\n")
    else:
    ...
    try:
      while not coord.should_stop():
        ...
        if FLAGS.segment_labels:
          results = get_segments(np.roll(video_batch_val, shift=FLAGS.shift, axis=1), num_frames_batch_val, 5)
      ...
      coord.request_stop()
      #'''
      if FLAGS.segment_labels and not FLAGS.output_prob:
