...
class CrossEntropyLoss(BaseLoss):
  ...
  def calculate_loss(self,
    ...
    with tf.name_scope("loss_xent"):
      ...
      k = tf.argmax(predictions, 1)
      num_examples = tf.cast(tf.shape(predictions)[0], dtype=k.dtype)
      idx = tf.stack([tf.range(num_examples), k], axis=-1)
      predictions = tf.gather_nd(predictions, idx)
      float_labels = tf.gather_nd(float_labels, idx) 
